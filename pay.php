...
$days=(strtotime($_SESSION['date2']) - strtotime($_SESSION['date1'])) / (60*60*24);
$tamt=$_SESSION['price']* $days;
?>
<!DOCTYPE html>
<html>
<head>
...
<div class= "app-container">
    ...
    <p><span class="left-icon"><img style="width:80px; height:40px; " src="https://upload.wikimedia.org/wikipedia/en/8/89/Razorpay_logo.svg"/></span>
      <span class="right-icon">Aramane</span></p>
    </div>
    
    <div class="middle-box">
        <h1><?php echo $tamt; ?>.00<span>INR</span></h1>
        <p>Pay To Aramane.</p>
    </div>
  ...
  <form method="POST" action="#">
    ...
    <input type="text" name="cardnumber" class="card-num-field" required="required" pattern="[0-9]*" maxlength="16" placeholder="xxxx-xxxx-xxxx-xxxx">
    </div>
    
    <div class="date-field-group">
    <label>Expiry Date</label><br>
    <input type="text" name="mm" class="date-field" required="required" pattern="[0-9]*" maxlength="2" placeholder="mm">
    <input type="text" name="yyyy" class="date-field" required="required" pattern="[0-9]*" maxlength="4" placeholder="yyyy">
    </div>
...
<!--    <button type="button" class="pay-btn">Pay Now</button> -->
	...
	{
		include("config.php");
		$date1=date("Y-m-d", strtotime($_SESSION['date1']));
		$date2=date("Y-m-d", strtotime($_SESSION['date2']));
		$query = "INSERT INTO `hbooking`(`name`, `date1`, `date2`, `adults`, `children`, `phone`, `address`, `email`, `room`, `hid`, `tamt`, `status`,`type`,`image`) VALUES ('".$_SESSION['guestname']."','".$date1."','".$date2."','".$_SESSION['adults']."','".$_SESSION['children']."','".$_SESSION['phone']."','".$_SESSION['address']."','".$_SESSION['user']."','".$_SESSION['room']."','".$_SESSION['hid']."','".$tamt."','booked and Paid','".$_SESSION['type']."','".$_SESSION['image']."')";
